...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    Var[Int](MaxTweetLength - tweetLength(tweetText()))
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] =
    Signal(
      remainingCharsCount() match {
        case fifteenOrMore if(fifteenOrMore >= 15) => "green"
        case zeroToFourteen if(zeroToFourteen >= 0 && zeroToFourteen < 15) => "orange"
        case _ => "red"
      }
    )
