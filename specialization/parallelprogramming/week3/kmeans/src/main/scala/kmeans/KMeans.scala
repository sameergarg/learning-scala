...
class KMeans {
  ...
  def findClosest(p: Point, means: GenSeq[Point]): Point = {
    ...
    println(s"Closest to point $p is mean:$closest")
    closest
  }

  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    /*if(points.isEmpty)
      means.map(_ -> List()).toMap
    else{
      points.map { point =>
    ...
    val meanToClosestPoints = points.par.groupBy(findClosest(_, means))
    means.par.map(mean => mean -> meanToClosestPoints.getOrElse(mean, GenSeq.empty)).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = oldMeans.zip(newMeans).find{
