...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      promise.success(List.empty[T])
      val start = promise.future
      ...
      /*val init = Future{List.empty[T]}
      fs.foldLeft(init){(acc, rem) =>
        for {
          f <- rem //Future[List[T]]
          a <- acc //Future[T]
        } yield(f::a)
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.foreach {p.completeWith(_) }
      // or fs.foreach {_ onComplete completeFirst }
      p.future*/

      Future.firstCompletedOf(fs) // using inbuilt method
